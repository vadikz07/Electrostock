...
class EditItemForm:
    ...
    def __init__(self, par, uuid_to_modify, popmanagerRef=None) -> None:
        ...
        self.root = par
        self.original_data = popmanagerRef.retrieve_item(
            uuid_target=self.uuid_to_modify
        )
        ...
        self.header_lbl = tb.Label(
            master=self.newWindow, text="Modo Edicion", font=("Arial", 12, "bold")
        )
        ...
        self.maker_lblframe = tb.LabelFrame(
            master=self.main_container, text="Fabricante"
        )
        ...
        self.cant_act_entry_var = tb.Spinbox(
            master=self.cant_act_lblframe, from_=1, to=200
        )
        ...
        self.clear_data_btn = tb.Button(
            ...
            bootstyle="info",
            command=lambda: (self.collect_data(), popmanagerRef.visualize_all()),
        )
    ...
    def collect_data(self) -> dict:
        ...
        if validateData(data_dict):
            modify_item(
                uuid_to_modify=self.uuid_to_modify,
                new_data=data_dict,
